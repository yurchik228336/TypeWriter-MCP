...
import { TypewriterSocketClient, parseConnectUrl, type TWPage } from "./socket-client.js";
...
let socket: TypewriterSocketClient | null = null;
if (!SOCKET_MODE && !FILE_MODE) {
  console.error(`Unsupported TYPEWRITER_MODE "${MODE}". Use "socket" or "file".`);
  process.exit(1);
}
const store: TypewriterStore | null = FILE_MODE ? new TypewriterStore(PAGES_DIR) : null;
...
async function readStoredConnectUrl(): Promise<string | undefined> {
  const raw = (await fs.readFile(CONNECT_URL_FILE, "utf8").catch(() => "")).trim();
  return raw || undefined;
}

async function persistConnectUrl(url: string): Promise<void> {
  await fs.mkdir(path.dirname(CONNECT_URL_FILE), { recursive: true });
  await fs.writeFile(CONNECT_URL_FILE, `${url.trim()}\n`, "utf8");
}

async function ensureSocketConnected(): Promise<TypewriterSocketClient> {
  if (!SOCKET_MODE) {
...
async function connectLive(url: string, forceReconnect = false): Promise<{ connected: true; pageCount: number; reusedExisting: boolean }> {
  if (!SOCKET_MODE) {
...
async function pages(): Promise<TWPage[]> {
  if (SOCKET_MODE) return await (await ensureSocketConnected()).fetchPages();
  const infos = await store!.listPages();
  const out: TWPage[] = [];
  for (const i of infos) out.push((await store!.readPage(i.name)) as unknown as TWPage);
  return out;
}
async function findPage(nameOrId: string): Promise<TWPage | undefined> {
...
server.tool("create_page", "Create a new page. Types: sequence, static, cinematic, manifest.",
  { name: z.string(), type: z.enum(["sequence", "static", "cinematic", "manifest"]), chapter: z.string().optional(), priority: z.number().optional() },
  async ({ name, type, chapter, priority }) => {
    try {
      if (SOCKET_MODE) return ok(await (await ensureSocketConnected()).createPage({ name, type, chapter, priority }));
...
server.tool("rename_page", "Rename a page.", { page: z.string(), newName: z.string() }, async ({ page, newName }) => {
  try {
    const p = await findPage(page);
    if (!p) throw new Error("Page not found: " + page);
    if (SOCKET_MODE) await (await ensureSocketConnected()).renamePage(p.id, newName); else await store!.renamePage(p.name, newName);
...
server.tool("delete_page", "Delete a page and all its entries.", { page: z.string() }, async ({ page }) => {
  try {
    const p = await findPage(page);
    if (!p) throw new Error("Page not found: " + page);
    if (SOCKET_MODE) await (await ensureSocketConnected()).deletePage(p.id); else await store!.deletePage(p.name);
    return ok("Deleted " + page);
  } catch (e) { return fail(e); }
});

server.tool("add_entry",
  ...
  { page: z.string(), fields: z.record(z.unknown()) },
  async ({ page, fields }) => {
    try {
      if (typeof fields.name !== "string" || typeof fields.type !== "string") throw new Error("fields must contain string 'name' and 'type'");
      const p = await findPage(page);
      if (!p) throw new Error("Page not found: " + page);
      if (SOCKET_MODE) return ok(await (await ensureSocketConnected()).createEntry(p.id, fields));
      return ok(await store!.addEntry(p.name, fields as { name: string; type: string }));
    } catch (e) { return fail(e); }
  });

server.tool("update_entry",
  "Update a single field of an entry by dotted path (e.g. 'text', 'speaker', 'criteria.0.value').",
  { page: z.string(), entryId: z.string(), path: z.string(), value: z.unknown() },
  async ({ page, entryId, path: fieldPath, value }) => {
    try {
      const p = await findPage(page);
      if (!p) throw new Error("Page not found: " + page);
      if (SOCKET_MODE) { await (await ensureSocketConnected()).updateEntryField(p.id, entryId, fieldPath, value); return ok("Updated " + fieldPath); }
      const patch: Record<string, unknown> = {}; patch[fieldPath] = value;
      return ok(await store!.updateEntry(p.name, entryId, patch));
    } catch (e) { return fail(e); }
  });

server.tool("replace_entry",
  "Replace a whole entry with a new object (must keep the same id).",
  { page: z.string(), entry: z.record(z.unknown()) },
  async ({ page, entry }) => {
    try {
      const p = await findPage(page);
      if (!p) throw new Error("Page not found: " + page);
      if (typeof entry.id !== "string") throw new Error("entry must include its 'id'");
      if (SOCKET_MODE) { await (await ensureSocketConnected()).updateCompleteEntry(p.id, entry as unknown as TWPage["entries"][number]); return ok("Replaced " + entry.id); }
...
server.tool("delete_entry", "Delete an entry by id.", { page: z.string(), entryId: z.string() }, async ({ page, entryId }) => {
  try {
    const p = await findPage(page);
    if (!p) throw new Error("Page not found: " + page);
    if (SOCKET_MODE) await (await ensureSocketConnected()).deleteEntry(p.id, entryId); else await store!.deleteEntry(p.name, entryId);
    return ok("Deleted entry " + entryId);
  } catch (e) { return fail(e); }
});

server.tool("connect_entries",
  "Wire story flow: append 'toEntryId' to the 'triggers' list of 'fromEntryId'.",
  { fromPage: z.string(), fromEntryId: z.string(), toEntryId: z.string() },
  async ({ fromPage, fromEntryId, toEntryId }) => {
    try {
      const p = await findPage(fromPage);
      if (!p) throw new Error("Page not found: " + fromPage);
      const from = (p.entries ?? []).find((e) => e.id === fromEntryId);
      if (!from) throw new Error("Entry not found: " + fromEntryId);
      const triggers = Array.isArray(from.triggers) ? (from.triggers as string[]).slice() : [];
      if (!triggers.includes(toEntryId)) triggers.push(toEntryId);
      if (SOCKET_MODE) await (await ensureSocketConnected()).updateEntryField(p.id, fromEntryId, "triggers", triggers);
      else await store!.updateEntry(p.name, fromEntryId, { triggers });
...
server.tool("search_entries", "Full-text search across all pages/entries.", { query: z.string() }, async ({ query }) => {
  try {
    ...
    const hits: { page: string; entry: unknown }[] = [];
    for (const p of await pages()) for (const e of p.entries ?? [])
      if (JSON.stringify(e).toLowerCase().includes(q)) hits.push({ page: p.name, entry: e });
    return ok(hits);
  } catch (e) { return fail(e); }
});
...
server.tool("validate_storyline", "Find broken references (triggers/triggeredBy pointing to missing ids).", {}, async () => {
  try {
    ...
    for (const p of all) for (const e of p.entries ?? []) ids.add(e.id);
    const problems: string[] = [];
    for (const p of all) for (const e of p.entries ?? []) {
      for (const f of ["triggers", "triggeredBy"]) {
        const v = (e as Record<string, unknown>)[f];
        if (Array.isArray(v)) for (const r of v)
          if (typeof r === "string" && r && !ids.has(r)) problems.push(`"${e.name ?? e.id}" (page ${p.name}) -> missing "${r}" in ${f}`);
      }
    }
...
if (SOCKET_MODE) {
  server.tool("publish", "Publish staging changes to production (applies edits live in-game).", {}, async () => {
    try { const r = await (await ensureSocketConnected()).publish(); return ok(r.message || "Published"); } catch (e) { return fail(e); }
  });
}
